...
theta = np.ones(n)
norm_dthetahats = np.ones(
    (len(alpha_facs), len(str_traces), 2, 1 + max_iter // big_step)
)
...
for t, alpha_fac in enumerate(alpha_facs):
    ...
    lrs = [1 / L, 2 / (mu + L)]
    wsklearn = np.linalg.solve(
        X.T @ np.diag(theta) @ X + alpha * np.eye(p), X.T @ np.diag(theta) @ y
    )
    jac_gt = np.linalg.solve(
        X.T @ np.diag(theta) @ X + alpha * np.eye(p), -np.diag(wsklearn) @ (X.T * X.T)
    )

    for j, str_trace in enumerate(str_traces):
        for r, lr in enumerate(lrs):
            what, ws, dthetahats, ltimes = diff_gd(
                np.zeros(p),
                theta,
                lr,
                alpha=alpha,
                start_trace=str_trace,
                max_iter=max_iter,
            )
            norm_ws[t, j, r, :] = np.linalg.norm(
                ws - wsklearn, axis=1
            ) / np.linalg.norm(wsklearn)
            if str_trace == max_iter - 1:
                tid0 = time.time()
                did = np.linalg.solve(
                    X.T @ np.diag(theta) @ X + alpha * np.eye(p),
                    -np.diag(what) @ (X.T * X.T),
                )
                dthetahats[-1, :, :] = did
                tid = time.time() - tid0
            else:
                tid = 0
            norm_dthetahats[t, j, r, :] = np.linalg.norm(
                dthetahats - jac_gt, axis=(1, 2)
            ) / np.linalg.norm(jac_gt)
...
for t, alpha_fac in enumerate(alpha_facs):
    for r, lr in enumerate(lrs):
        for j, str_trace in enumerate(str_traces):
            axs[t, r].scatter(
                times[t, j, r, :].T, norm_dthetahats[t, j, r, :].T, color=colors[j]
            )
            axs[t, r].plot(
                times[t, j, r, :].T,
                norm_dthetahats[t, j, r, :].T,
                label="_nolegend_",
                color=colors[j],
            )
            axs[t, r].scatter(
                times[t, j, r, :].T, norm_ws[t, j, r, :].T, marker="x", color=colors[j]
            )
            axs[t, r].plot(
                times[t, j, r, :].T,
                norm_ws[t, j, r, :].T,
